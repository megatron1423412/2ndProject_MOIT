...
const steps: FlowStep[] = [
  ...
  },
  {
    id: "ac-intro-2",
    type: "assistant-message",
    message: "설치 공간과 사용 방식에 딱 맞는 에어컨을 찾아드릴까요? 💡 딱 맞는 최적의 솔루션을 찾아드릴 테니, 먼저 간단한 정보부터 차근차근 확인해 볼게요! 👍",
    next: "ac-intro-3",
  },
  {
    id: "ac-intro-3",
    type: "assistant-message",
    message: "첫 번째 단계로, 에어컨을 주로 어디에 설치하실 예정인지 살짝 확인해 볼게요! 🏠💡",
    next: "ac-space",
  },
  {
  ...
  { id: "ac-area-direct", type: "number-input", message: "실제로 시원하게 만들 공간은 몇 평 정도인가요?", answerKey: `${n}.actualCoolingArea`, min: 1, max: 100, unit: "평", placeholder: "냉방 공간 평수로 입력해주세요.", alternateOption: { value: "unknown", label: "잘 모르겠어요", next: "ac-home" }, next: "ac-usage" },
  { id: "ac-home", type: "number-input", message: "그럼 집 전체는 몇 평인지도 알려주시겠어요? 📏", answerKey: `${n}.homePyeong`, min: 1, max: 200, unit: "평", placeholder: "집 전체 평수로 입력해주세요.", next: "ac-area-info" },
  {
  ...
  {
    id: "ac-area-mode", type: "single-choice", message: "입력해 주신 평수를 냉방 면적으로 적용해 드릴까요? 😊", answerKey: `${n}.coolingAreaMode`, options: [
      { value: "recommended", label: "계산값 적용", next: "ac-usage" },
      { value: "custom", label: "직접 수정", next: "ac-area-custom" },
    ]
  },
  { id: "ac-area-custom", type: "number-input", message: "좋아요! 필요한 최소 냉방 면적을 직접 입력해 주세요 ✍️", answerKey: `${n}.customCoolingArea`, min: 1, max: 100, unit: "평", placeholder: "최소 냉방 면적을 입력해주세요.", next: "ac-usage" },
  {
  ...
  { id: "ac-budget", type: "number-input", message: "에어컨에 쓰실 최대 예산은 얼마 정도일까요? 딱 맞춤 구성으로 안내해 드릴게요! 🙌", answerKey: `${n}.budget`, min: 1, unit: "원", placeholder: "생각하고 계신 최대 가격을 입력해 주세요.", alternateOption: { value: "none", label: "예산 제한 없음", next: "ac-summary" }, next: "ac-summary" },
  {
    id: "ac-summary", type: "assistant-message", message: "지금까지 알려주신 조건을 한눈에 정리해 드렸어요! 📋", buildMessage: (answers) => {
      ...
      const formattedBudget = budget === "none" ? "예산 제한 없음" : `${Number(budget).toLocaleString("ko-KR")}원 이하`;
      ...
      return [
        ...
        `• **제품 가격**: ${formattedBudget}`,
      ].join("\n");
    }, next: "ac-auto-rules"
  },
  { id: "ac-auto-rules", type: "assistant-message", message: "**모잇이 자동으로 챙겨드리는 것들이에요! ✨**\n• 냉방 면적이 부족한 제품은 미리 정리해 드려요\n• 전기세 지켜드리는 인버터 제품만 챙겨드려요 💰\n• 판매 중단된 제품은 미리 정리해 드려요\n• 현재 가격, 과거 가격 흐름, 에너지 등급, 자동 건조, 사용 시간, 선택하신 가성비 기준까지 꼼꼼하게 따져서 순위를 매겨요 🔍", next: "ac-confirm" },
  { id: "ac-confirm", type: "confirmation", message: "이 조건으로 추천을 시작해 볼까요?", answerKey: `${n}.confirmed`, confirmLabel: "시작해 주세요", cancelLabel: "조건을 수정할게요", confirmNext: "ac-result", cancelNext: "$restart" },
