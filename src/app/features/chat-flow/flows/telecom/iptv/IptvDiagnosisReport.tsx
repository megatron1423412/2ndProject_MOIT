...
export default function IptvDiagnosisReport({ result }: IptvDiagnosisReportProps) {
  ...
  const penalty12 = Math.round(currentPriceInput * 3.5);
  const penalty24 = Math.round(currentPriceInput * 7.5);
  const bep12 = savingDiff > 0 ? Math.ceil(penalty12 / savingDiff) : null;
  const bep24 = savingDiff > 0 ? Math.ceil(penalty24 / savingDiff) : null;
