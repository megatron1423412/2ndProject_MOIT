...
const specific: FlowStep[] = [
  ...
  // [Part 3 - 12번] 소비 패턴 등급 진단 여부 분기 질문
  {
    ...
    options: [
      ...
      { value: "no", label: "NO", next: "bundle-exit" },
    ],
  },
  ...
  // [Part 3 - 13번] 결합상품 소비 패턴 등급 진단 결과 노출
  {
    ...
    next: "bundle-ask-share",
  },

  // [Part 3 - 14번] SNS 공유 의사 확인 질문
  {
    id: "bundle-ask-share",
    type: "single-choice",
    ...
    options: [
      { value: "yes", label: "YES", next: "bundle-sns-redirect" },
      { value: "no", label: "NO", next: "bundle-exit" },
    ],
  },

  // [Part 3 - 15번] SNS 공유 리다이렉트
  {
    id: "bundle-sns-redirect",
    type: "assistant-message",
    message: "인스타그램으로 이동합니다. [여기를 클릭하여 인스타그램에서 결과를 공유](https://instagram.com)해 주세요.",
    next: "bundle-exit",
  },

  // [Part 3 - 16번] 종료 및 새로운 주제 시작 질문 스텝
  {
    id: "bundle-exit",
    type: "single-choice",
    ...
    options: [],
    optionsResolver: () => [
      { value: "restart", label: "예, 새로운 주제로 시작할래요", next: "bundle-intro" },
      { value: "exit", label: "아니요, 대화를 종료할래요", next: "bundle-completed-exit" },
    ],
  ...
  // [Part 3 - 17번] 최종 대화 종료
  {
    ...
    type: "result",
    message: "결합상품 요금제 진단 서비스를 이용해 주셔서 감사합니다. 안전하게 대화가 종료되었습니다.",
  },
];
