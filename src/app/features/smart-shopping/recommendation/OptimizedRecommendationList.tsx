...
export default function OptimizedRecommendationList({ items, catalogSource, onSelect, isFavorite, onToggleFavorite, isActive = true }: Props) {
  ...
  return (
    <section className="min-w-0 rounded-xl border border-border bg-card p-4 shadow-sm" data-chat-content="ai-reorder">
      <div><p className="text-xs font-black text-accent">MOIT 상품 DB</p><h3 className="mt-1 text-base font-black text-primary">AI 최적화 재정렬</h3><p className="mt-1 text-xs text-muted-foreground">코드의 필수 필터와 적합도 점수 결과이며 AI가 임의로 정한 순위가 아닙니다.</p></div>
      <div className="mt-4 space-y-2">
        {items.length === 0 && <p className="rounded-lg bg-muted/30 p-4 text-sm text-muted-foreground">필수 조건을 만족한 상품 데이터가 없습니다.</p>}
