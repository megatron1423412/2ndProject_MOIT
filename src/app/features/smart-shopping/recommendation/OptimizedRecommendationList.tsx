...
export default function OptimizedRecommendationList({ items, onSelect, isActive = true }: { items: ProductRecommendation[]; onSelect: (item: ProductRecommendation) => void; isActive?: boolean }) {
  return (
    <section className="min-w-0 rounded-xl border border-border bg-card p-4 shadow-sm">
      ...
      <div className="mt-4 space-y-2">
        ...
        {items.slice(0, 10).map((item, index) => (
          <button key={item.product.id} type="button" disabled={!isActive} onClick={() => onSelect(item)} className="flex w-full items-start gap-3 rounded-lg border border-border p-3 text-left transition hover:border-accent hover:bg-muted/25 focus:outline-none focus:ring-2 focus:ring-accent/40 disabled:cursor-default disabled:opacity-75">
