...
export default function NaverLowestPriceList({ items, status, errorMessage, onRetry, onSelect }: Props) {
  return (
    <section className="min-w-0 rounded-xl border border-border bg-card p-4 shadow-sm">
      ...
      {status === "error" && <div className="mt-4 rounded-lg border border-amber-300/50 bg-amber-50 p-4 dark:bg-amber-400/10"><p className="text-sm font-bold text-amber-800 dark:text-amber-200">{errorMessage}</p><button type="button" onClick={onRetry} className="mt-3 rounded-lg bg-primary px-3 py-2 text-xs font-black text-primary-foreground">다시 시도</button></div>}
      ...
      <div className="mt-4 space-y-2">
        {status === "success" && items.map((item, index) => (
          <button key={item.productId} type="button" onClick={() => onSelect(item)} className="flex w-full items-start gap-3 rounded-lg border border-border p-3 text-left transition hover:border-accent hover:bg-muted/25 focus:outline-none focus:ring-2 focus:ring-accent/40">
