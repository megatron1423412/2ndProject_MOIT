...
function NaverProductDetail({ selected, internalRecommendations, isFavorite, onToggleFavorite }: { selected: Extract<SelectedShoppingProduct, { source: "naver" }>; internalRecommendations: ProductRecommendation[]; isFavorite: boolean; onToggleFavorite?: () => void }) {
  ...
  return (
    <article className="rounded-xl border border-border bg-card p-5 shadow-sm">
      ...
      <div className="mt-3">{internal ? <><PriceTrendMiniChart history={internal.priceHistory} /><p className="mt-1 text-[10px] font-bold text-amber-700 dark:text-amber-300">그래프는 모잇 내부 카탈로그 가격 이력입니다.</p></> : <p className="rounded-lg bg-muted/30 p-4 text-xs text-muted-foreground">가격 이력 없음</p>}</div>
