...
function NaverProductDetail({ selected, internalRecommendations, categoryId, isFavorite, onToggleFavorite }: { selected: Extract<SelectedShoppingProduct, { source: "naver" }>; internalRecommendations: ProductRecommendation[]; categoryId?: ProductCategoryId; isFavorite: boolean; onToggleFavorite?: () => void }) {
  ...
  return (
    <article className="rounded-xl border border-border bg-card p-5 shadow-sm">
      <div className="flex flex-col gap-4 sm:flex-row"><ImageWithFallback src={naver.imageUrl} alt={`${naver.title} 네이버 쇼핑 이미지`} className="h-40 w-40 flex-none rounded-lg border border-border bg-muted object-cover" /><div className="min-w-0 flex-1"><div className="flex items-start justify-end">{onToggleFavorite && <FavoriteToggleButton isFavorite={isFavorite} onToggle={onToggleFavorite} positionClassName="relative right-auto top-auto" />}</div><p className="mt-3 text-xs font-bold text-muted-foreground">{naver.brand || naver.maker || "브랜드 정보 없음"}{internal ? ` · ${internal.modelNumber}` : ""}</p><h3 className="mt-1 text-lg font-black text-primary">{naver.title}</h3><p className="mt-2 text-xs text-muted-foreground">{internal?.shortInfo ?? "모잇 DB 검증 정보 없음"}</p>{recommendation && <p className="mt-2 text-sm font-black text-accent">조건 적합도 {recommendation.score}점</p>}</div></div>
      <div className="mt-4"><CriteriaMatchList matched={recommendation?.matchedCoreCriteria ?? (internal ? ["모델번호 기준 내부 DB 매칭"] : [])} unmatched={confirmationItems} confirmationTitle={isAirConditioner ? "구매 전 확인" : undefined} /></div>
