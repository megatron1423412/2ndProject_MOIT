...
function NaverProductDetail({ selected, internalRecommendations }: { selected: Extract<SelectedShoppingProduct, { source: "naver" }>; internalRecommendations: ProductRecommendation[] }) {
  ...
  return (
    <article className="rounded-xl border border-border bg-card p-5 shadow-sm">
      <div className="flex flex-col gap-4 sm:flex-row"><ImageWithFallback src={naver.imageUrl} alt={`${naver.title} 네이버 쇼핑 이미지`} className="h-40 w-40 flex-none rounded-lg border border-border bg-muted object-cover" /><div className="min-w-0 flex-1"><div className="flex flex-wrap gap-2"><span className="rounded bg-emerald-50 px-2 py-1 text-[10px] font-black text-emerald-700 dark:bg-emerald-400/10 dark:text-emerald-300">네이버 쇼핑</span>{internal && <span className="rounded bg-brand-surface px-2 py-1 text-[10px] font-black text-brand-surface-foreground">모잇 DB 모델 매칭</span>}</div><p className="mt-3 text-xs font-bold text-muted-foreground">{naver.brand || naver.maker || "브랜드 정보 없음"}{internal ? ` · ${internal.modelNumber}` : ""}</p><h3 className="mt-1 text-lg font-black text-primary">{naver.title}</h3><p className="mt-2 text-xs text-muted-foreground">{internal?.shortInfo ?? "모잇 DB 검증 정보 없음"}</p>{recommendation && <p className="mt-2 text-sm font-black text-accent">조건 적합도 {recommendation.score}점</p>}</div></div>
