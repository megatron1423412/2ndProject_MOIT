...
export default function RecommendationSelectionView({ result, supplementalMessages, onAppendSupplementalMessage, onClearSupplementalMessages, onEndSmartShoppingChat, onCreatePriceAlert, userId }: Props) {
  ...
  useEffect(() => {
    ...
    dispatch({ type: "complete-purchase-grade", result: calculatePurchaseGrade(view.purchaseGradeInput.currentPrice, view.purchaseGradeInput.allTimeLow) });
  }, [view]);
  ...
  const backToList = () => { setQuestionOpen(false); onClearSupplementalMessages(); dispatch({ type: "back-to-list" }); };
  const startGrade = () => {
    if (!selected) return;
    dispatch({ type: "start-purchase-grade", input: startPurchaseGradeDiagnosis({ selected, recommendations: result.recommendations ?? [], userCriteria: criteria }) });
  };
  const handleNextAction = (action: NextActionId) => {
    if (!selected) return;
    if (action === "purchase-grade") return startGrade();
    if (action === "purchase-link") return dispatch({ type: "confirm-purchase-link", purchaseLink: resolvePurchaseLink(selected, naverItems) });
    if (action === "price-alert") return dispatch({ type: "set-price-alert" });
    if (action === "back-to-list") return backToList();
    endSmartShoppingChat(onEndSmartShoppingChat);
  };
  const savePriceAlert = (targetPrice: number) => {
    ...
    onCreatePriceAlert({ userId, productId, productName, modelNumber: product?.modelNumber, source: selected.source, purchaseLink, currentPrice, targetPrice, active: true });
    onAppendSupplementalMessage({ sender: "user", text: "최저가 알람 설정" });
    onAppendSupplementalMessage({ sender: "ai", text: `${productName}의 목표 가격을 ${targetPrice.toLocaleString("ko-KR")}원으로 저장했어요. 상시 감시는 아직 연결되지 않았으며, 가격을 다시 확인할 때 조건을 평가합니다.` });
    dispatch({ type: "return-to-next-actions" });
  };

  if (view.stage === "grading-purchase") return <div className="rounded-xl border border-border bg-card p-4 text-sm font-bold text-muted-foreground" data-stage="grading-purchase">가격 기록을 기준으로 구매 타이밍을 계산하고 있어요…</div>;
  if (view.stage === "grade-complete") return <div className="space-y-3" data-stage="grade-complete"><PurchaseGradeResultCard input={view.purchaseGradeInput} result={view.purchaseGradeResult} /><NextActionSelection showPurchaseGrade={false} onSelect={handleNextAction} /></div>;
  if (view.stage === "choosing-next-action") return <NextActionSelection onSelect={handleNextAction} />;
  if (view.stage === "confirming-purchase-link") return <PurchaseLinkAction link={view.purchaseLink} onCancel={() => dispatch({ type: "return-to-next-actions" })} />;
  if (view.stage === "setting-price-alert") {
    const product = selected?.source === "internal" ? selected.recommendation.product : selected?.matchedInternalProduct;
    const currentPrice = selected?.source === "internal" ? selected.recommendation.product.currentPrice : selected?.source === "naver" ? selected.product.lowestPrice : 0;
    const allTimeLow = product?.priceHistory.length ? Math.min(...product.priceHistory.map((point) => point.lowestPrice)) : undefined;
    return <PriceAlertForm productName={product?.name ?? (selected?.source === "naver" ? selected.product.title : "상품명 없음")} currentPrice={currentPrice} allTimeLow={allTimeLow} onSubmit={savePriceAlert} onCancel={() => dispatch({ type: "return-to-next-actions" })} />;
  }
  if (view.stage === "viewing-product-detail" && selected) {
    return <ProductDetailView selected={selected} internalRecommendations={result.recommendations ?? []} messages={supplementalMessages} showAlternative={showAlternative} questionOpen={questionOpen} questionLoading={questionLoading} questionError={questionError} onAction={handleAction} onQuestionSubmit={(question) => void handleQuestionSubmit(question)} onQuestionRetry={(question) => void handleQuestionSubmit(question, false)} onQuestionCancel={() => { setQuestionOpen(false); setQuestionError(""); }} onBack={backToList} onNext={() => dispatch({ type: "choose-next-action" })} />;
  }
