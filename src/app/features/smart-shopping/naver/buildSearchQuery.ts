...
import { calculateRecommendedCoolingArea } from "../../chat-flow/flows/appliances/air-conditioner/criteria";
...
export const buildAirConditionerSearchQuery = (answers: FlowAnswers) => {
  const type = String(answers["airConditioner.type"] ?? "");
  const typeLabel = { standing: "스탠드", wall: "벽걸이", "two-in-one": "2in1", window: "창문형" }[type] ?? "";
  const area = answers["airConditioner.coolingAreaMode"] === "custom"
    ? Number(answers["airConditioner.customCoolingArea"])
    : calculateRecommendedCoolingArea(Number(answers["airConditioner.homePyeong"]));
